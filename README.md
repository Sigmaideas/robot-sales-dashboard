...
### 자동 저장
- 모든 추가/이동/삭제/초기화는 즉시 저장됩니다.
- **공유 저장소(JSONBin)를 설정하면** 모든 접속자가 같은 데이터를 보고, 한 사람의 변경이 모두에게 반영됩니다.
- 공유 저장소를 설정하지 않으면 기존처럼 이 브라우저의 `localStorage`에만 저장됩니다.
  - 키 형식: `salesData.<연도>` (예: `salesData.2026`)

## 여러 사람과 데이터 공유하기 (JSONBin)

기본 상태에서는 데이터가 각자 브라우저에만 저장되어 서로 공유되지 않습니다.
무료 서비스인 [JSONBin.io](https://jsonbin.io)를 공용 저장소로 연결하면 모두가 같은 데이터를 보게 됩니다.

### 1. JSONBin 가입 & Bin 만들기
1. https://jsonbin.io 에서 무료 회원가입 후 로그인합니다.
2. 좌측 메뉴 **BINS → CREATE BIN** 으로 새 Bin을 만듭니다.
3. 내용으로 빈 객체 `{}` 를 넣고 저장합니다.
4. 주소창이나 Bin 정보에 있는 **Bin ID** (`6xxxxxxxxxxxxxxxx` 형태)를 복사해 둡니다.

### 2. Access Key 발급 (보안용)
1. 좌측 메뉴 **API KEYS → ACCESS KEYS → CREATE ACCESS KEY**.
2. 권한은 **Bins: Read, Update 만 체크**(Create/Delete는 끄기)하고 생성합니다.
3. 생성된 키(`$2a$10$...` 형태)를 복사해 둡니다.

### 3. 코드에 키 입력
`script.js` 상단의 `CONFIG`에 위 두 값을 채워 넣습니다.

```js
const CONFIG = {
  BIN_ID: '여기에_Bin_ID',
  ACCESS_KEY: '여기에_Access_Key',
};
```

저장(또는 GitHub에 push)하면 접속자 전원이 공용 데이터를 공유합니다.

### 동작 방식 / 주의
- 추가·이동·삭제·이름변경·초기화는 즉시 공용 저장소에 저장됩니다(헤더에 "동기화됨" 표시).
- 다른 사람의 변경은 **탭을 다시 보거나(포커스)** 헤더의 **새로고침** 버튼을 누르면 반영됩니다(실시간 자동 갱신은 아님).
- 두 사람이 *거의 동시에* 편집하면 나중에 저장한 쪽이 덮어쓸 수 있습니다. 가능하면 작업 전 새로고침을 권장합니다.
- 키는 정적 사이트 특성상 코드에 노출되므로, 반드시 **Read/Update 권한만 가진 Access Key**를 사용하세요(사내용 데이터라 위험도는 낮음).
